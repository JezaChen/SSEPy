...
class SSE1(schemes.interface.inverted_index_sse.InvertedIndexSSE):
    ...
    def _Enc(self, K: SSE1Key, database: dict) -> SSE1EncryptedDatabase:
        ...
        for keyword in database:
            ...
            for j, identifier in enumerate(database[keyword],
                                           start=1):  # j begins from 1
            ...
            last_node_addr_in_A = self.config.prp_psi(
                Bitset(K1, length=self.config.param_k_bits),
                Bitset(ctr, length=self.config.param_log2_s))

            A[int(last_node_addr_in_A)] = self.config.ske1.Encrypt(
                K_i[-1], last_node)
        ...
        existing_entry_size = len(
            self.config.ske1.Encrypt(
                b"\x00" * self.config.param_k, b"\x00" *
                (self.config.param_identifier_size + self.config.param_k +
        ...
        for i in range(len(A)):
            if A[i] == b'\x00':
                A[i] = os.urandom(
                    existing_entry_size
                )  # the same size as the existing s' entries of A

        # Fill random values to T so that |T| = |∆|
        for _ in range(self.config.param_dictionary_size - len(T)):
            T[os.urandom(self.config.param_l)] = os.urandom(
                self.config.prf_f.output_length)
    ...
    def _Trap(self, K: SSE1Key, keyword: bytes) -> SSE1Token:
        ...
        return SSE1Token(
            bytes(
                self.config.prp_pi(
                    Bitset(K3, length=self.config.param_k_bits),
                    Bitset(keyword, length=self.config.param_l_bits))),
            self.config.prf_f(K2,
                              add_leading_zeros(keyword, self.config.param_l)))
    ...
    def _Search(self, I: SSE1EncryptedDatabase, t: SSE1Token) -> SSE1Result:
        ...
        node_addr, K_prime = split_bytes_given_slice_len(
            xor_result, [self.config.param_log2_s_bytes, self.config.param_k])
        result = []
        while True:
            node_cipher = A[int_from_bytes(node_addr)]
            node = self.config.ske1.Decrypt(K_prime, node_cipher)
            # parse the node
            file_identifier, next_key, next_node_addr = split_bytes_given_slice_len(
                node, [
                    self.config.param_identifier_size, self.config.param_k,
                    self.config.param_log2_s_bytes
                ])
            ...
            if K_prime == b'\x00' * len(
                    K_prime) and node_addr == b'\x00' * len(node_addr):
                break
