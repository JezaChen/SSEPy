...
class Pi2Lev(schemes.interface.inverted_index_sse.InvertedIndexSSE):
    ...
    def _Enc(self, K: Pi2LevKey, database: dict) -> Pi2LevEncryptedDatabase:
        ...
        for keyword in database:
            if len(database[keyword]) > self.config.param_b:
                A_len += math.ceil(
                    len(database[keyword]) / self.config.param_B)
            if len(database[keyword]
                   ) > self.config.param_b_prime * self.config.param_B:
                A_len += math.ceil(
                    len(database[keyword]) /
                    (self.config.param_B * self.config.param_B_prime))
        ...
        A = [None] * A_len
        available_pos_list = random.sample(range(1, A_len), A_len -
                                           1)  # index of A start at 1 !!
        ...
        for keyword in database:
            ...
            if len(database[keyword]) <= self.config.param_b:  # DB(w) is small
                ...
                d = self.config.ske.Encrypt(K2,
                                            LEVEL_FILE_IDENTIFIER + db_w_bytes)
                L.append((l, d))

            elif self.config.param_b < len(
                    database[keyword]
            ) <= self.config.param_B * self.config.param_b_prime:
                # DB(w) is medium
                ...
                for j, file_id_block in enumerate(file_id_block_list):
                    ...
                    )  # Choose random empty index
                    index_list_in_A.append(
                        int_to_bytes(index_in_A, output_len=index_size_in_A))

                    d = self.config.ske.Encrypt(
                        K2, LEVEL_FILE_IDENTIFIER + file_id_block)
                    A[index_in_A] = d
                ...
                index_of_A_block_bytes += b'\x00' * (
                    dict_block_size - len(index_of_A_block_bytes))
                l = self.config.prf_f(K1, b"\x00")
                d = self.config.ske.Encrypt(
                    K2, LEVEL_POINTER_OF_ARRAY + index_of_A_block_bytes)
                L.append((l, d))

            elif self.config.param_B * self.config.param_b_prime < len(
                    database[keyword]) < (
                        self.config.param_B *
                        self.config.param_B_prime) * self.config.param_b_prime:
                ...
                for j, file_id_block in enumerate(file_id_block_list):
                    ...
                    first_level_index_list_in_A.append(
                        int_to_bytes(index_in_A, output_len=index_size_in_A))
                ...
                first_level_index_block_list = partition_identifiers_to_blocks(
                    first_level_index_list_in_A,
                ...
                for j, first_index_id_block in enumerate(
                        first_level_index_block_list):
                    # store first level index blocks in array A

                    index_in_A = available_pos_list.pop(
                    )  # Choose random empty index
                    second_level_index_list_in_A.append(
                        int_to_bytes(index_in_A, output_len=index_size_in_A))

                    d = self.config.ske.Encrypt(
                        K2, LEVEL_POINTER_OF_ARRAY + first_index_id_block)
                    A[index_in_A] = d

                # Encrypt and store a single block of second-level pointers in dictionary D:

                second_index_of_A_block_bytes = b''.join(
                    second_level_index_list_in_A)
                # Pad {i'_1, ..., i'_t} to b elements if necessary
                second_index_of_A_block_bytes += b'\x00' * (
                    dict_block_size - len(second_index_of_A_block_bytes))
                l = self.config.prf_f(K1, b"\x00")
                d = self.config.ske.Encrypt(
                    K2, LEVEL_POINTER_OF_ARRAY + second_index_of_A_block_bytes)
                L.append((l, d))
            else:
    ...
    def _Search(self, edb: Pi2LevEncryptedDatabase,
                tk: Pi2LevToken) -> Pi2LevResult:
        """Search Algorithm"""
        D = edb.D
        A = edb.A
        K1, K2 = tk.K1, tk.K2

        prev_level_result = [self.config.prf_f(K1, b'\x00')
                             ]  # from top level to search
        curr_level_result = []
        ...
        while not is_in_file_id_level:
            ...
            block_plaintext_list = [
                self.config.ske.Decrypt(K2, block_cipher)
            ...
            param_key_entry_num_in_one_block = param_by_level[
                curr_process_level][level_mark == LEVEL_POINTER_OF_ARRAY]
            param_val_entry_num_in_one_block = self.config[
                param_key_entry_num_in_one_block]
            ...
            for block_plaintext in block_plaintext_list:
                ...
                curr_level_result.extend(
                    parse_identifiers_from_block_given_entry_count_in_one_block(
                        block_content, param_val_entry_num_in_one_block))
