...
class PiPtr(schemes.interface.inverted_index_sse.InvertedIndexSSE):
    ...
    def _Enc(self, K: PiPtrKey, database: dict) -> PiPtrEncryptedDatabase:
        ...
        A_len = sum(
            ...
            for keyword in database) + 1
        A = [None] * A_len
        index_size_in_A = math.ceil(math.log2(A_len) /
                                    8)  # Fixed size of the index of A
    ...
    def _Search(self, edb: PiPtrEncryptedDatabase,
                tk: PiPtrToken) -> PiPtrResult:
        """Search Algorithm"""
        D = edb.D
        A = edb.A
        K1, K2 = tk.K1, tk.K2
        ...
        while True:
            ...
            index_list.extend(
                parse_identifiers_from_block_given_entry_count_in_one_block(
                    self.config.ske.Decrypt(K2, index_block_cipher),
                    self.config.param_b))
            c += 1

        for index_in_A in index_list:
            file_id_block_cipher = A[int_from_bytes(
                index_in_A)]  # need to convert bytes to int
