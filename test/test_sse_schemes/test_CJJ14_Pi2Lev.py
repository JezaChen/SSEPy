...
class TestPi2Lev(unittest.TestCase):
    ...
    def test_structure_serialization(self):
        ...
        scheme = Pi2Lev(config_dict)
        key = scheme.KeyGen()
        self.assertEqual(key, Pi2LevKey.deserialize(key.serialize(), scheme.config))

        encrypted_index = scheme.EDBSetup(key, db)
        self.assertEqual(encrypted_index,
                         Pi2LevEncryptedDatabase.deserialize(encrypted_index.serialize(), scheme.config))

        for keyword in db:
            token = scheme.TokenGen(key, keyword)
            self.assertEqual(token,
                             Pi2LevToken.deserialize(token.serialize(),
                                                     scheme.config))
            result = scheme.Search(encrypted_index, token)
            self.assertEqual(result,
                             Pi2LevResult.deserialize(result.serialize(),
                                                      scheme.config))
