...
class LubyRackoffPRP(AbstractPRP):
    ...
    def __init__(self, *, message_length: int, key_length: int,
                 underlying_prf: AbstractPRF):
        super(LubyRackoffPRP, self).__init__(message_length=message_length,
                                             key_length=key_length)
        if underlying_prf.key_length * 3 != key_length:
            raise ValueError(
                "The key length of the PRP needs to be 3 times the length of the underlying PRF key."
            )
        if underlying_prf.message_length != underlying_prf.output_length:
            raise ValueError(
                "The input length of the PRF needs to be equal to the output length."
            )
    ...
    def __call__(self, key: bytes, message: bytes) -> bytes:
        ...
        curr_left, curr_right = message[:self.message_length //
                                        2], message[self.message_length // 2:]
        # K0, K1, K2
        key_list = [
            key[i:i + self.key_length // 3]
            for i in range(0, self.key_length, self.key_length // 3)
        ]

        for i in range(3):
            next_left, next_right = curr_right, bytes_xor(
                curr_left, self.underlying_prf(key_list[i], curr_right))
